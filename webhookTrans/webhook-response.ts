...
import DFCard from "./responses/df-card";
import DFQuickReplies from "./responses/df-quickreplies";
import DFQuickRepliesInput from "./responses/df-quickrepliesinput";
import DFText from "./responses/df-text";
import MessagePayload, { CustomAdditionalPayload, CustomQuickReplies, CustomText } from "./responses/message-payload";
import WebhookPayload, { Google, GoogleCarousel, GoogleCarouselItem, GoogleItem, GoogleBasicCard,GoogleLinkOutSuggestion, GoogleListData, GoogleListIntent, GoogleListItem, GoogleOrderUpdate, GooglePermissionData, GooglePermissionIntent, GoogleSignInIntent, GoogleSimpleResponse, GoogleStructuredResponse, GoogleSuggestion, GoogleTableCards, GoogleTableCardsHeader, GoogleTableCardsItem, GoogleTableCardsItemCell, GoogleTransactionalContents, GoogleTransactionalLineItem, GoogleTransactionBuyerInfo, GoogleTransactionDecisionData, GoogleTransactionDecisionIntent, GoogleTransactionMerchant, ISystemIntent } from "./responses/webhook-payload";
import { OtherAction, SalesIQInput, SalesIQMessage, SalesIQPayload, SalesIQReplies, SalesIQResponse } from "./responses/zoho-responses";
import WebhookRequest, { IOutputContext } from "./webhook-request";
...
export default class WebhookResponse {
    ...
    sessionEntityTypes: ISessisonEntityType[] = []
     ...
     * @memberof WebhookResponse
     */
    constructor(webhookRequest: WebhookRequest, currentIntentName) {
        this.webhookRequest = webhookRequest
        this.messagePayload = new MessagePayload()
        this.webhookPayload = new WebhookPayload()
        this.googlePayload = new Google(),
    ...
    addDFMessage(dfMessage, isZohoSuggestion = false, generateSalesIQ = true, generateGAResponse = true, sendtodisplaydevice = false, dislikeaction = false, otheraction = false) {
        ...
        if (!(dfMessage instanceof SalesIQResponse)) {

            //If message is an instance of DFText or DFCard or DFQuickReplies add JSON response in responce message array
            if ((dfMessage instanceof DFText) || (dfMessage instanceof DFCard) || (dfMessage instanceof DFQuickReplies) || (dfMessage instanceof DFQuickRepliesInput)) {
                if (dfMessage instanceof DFText) {
                    dfMessage = this.updateVehicleNameInResponse(dfMessage)
                }
                this.dfMessages.push(dfMessage.getJSON())
            } else if (dfMessage instanceof CustomAdditionalPayload) { // If message is an instance of CustomAdditionalPayload add playload and platform
                this.dfMessages.push({ payload: dfMessage, platform: dfMessage.platform })
            } else if (dfMessage instanceof SalesIQPayload) { // If message is an instance of SalesIQPayload add message as Zoho Payload
            ...
            if (dfMessage instanceof DFText) {
                ...
                this.messagePayload.add(new CustomText(dfMessage.text, PLATFORM.FACEBOOK));
            ...
            */
            if (dfMessage instanceof DFQuickReplies) {
            ...
            if (dfMessage instanceof DFQuickRepliesInput) {
                if (generateSalesIQ) {
    ...
    addZohoQuickReplies(quickReplies, title = null,type='select',placeholder='',value='',error=[]) {
        ...
        if (!this.zohoResponse.payload.input) {
            this.zohoResponse.payload.input = new SalesIQInput()
        }
        //Add quick replies
    ...
    addZohoDislikeaction() {
        ...
        let dislikeaction = new OtherAction()
    ...
    addZohoOtherQuery() {
        ...
        let otheractions = new OtherAction()
    ...
    addGooglePayloadText(text, speech, sendtodisplaydevice) {
        ...
        if (googleSimpleResponsesItemsLength > 0) {
            ...
            if (googleSimpleResponsesItemsLength == 1 || this.webhookRequest.isWhatsappRequest()) {
                let googleItem = new GoogleItem()
                googleItem.simpleResponse = new GoogleSimpleResponse()
                let updatedTextToSpeechResponse: any = this.updateTexttoSpeechResponse(speech)
        ...
        if (sendtodisplaydevice) {
            this.googlePayload.systemIntent = new ISystemIntent("Let's move you to a screen device for cards and other visual responses", "Try your Action here!")
        }
    }
     ...
     * @memberof WebhookResponse
     */
    createSpeechResponseForGoogleHome(text, speech, options: string[], isAddress = false, shouldSpeakOptions: boolean | number = true, sendtodisplaydevice = false) {
...
*/
     ...
     * @returns
     */
    mergeSSML = (oldSSML: string, newSSML: string) => {
     ...
     * @memberof WebhookResponse
     */
    updateVehicleNameInResponse(dfMessage: DFText) {
        let cityName = null
        ...
        if (this.currentIntentName) {
            contextCity =
                ...
                this.webhookRequest.getParameterFromContext('contextCity', Constant.PRICECONSTANT.WELCOME_CONTEXT)
        }
        ...
        const vehicleCode = this.webhookRequest.getParameterFromContext('Vehicle', Constant.PRICECONSTANT.WELCOME_CONTEXT) || this.webhookRequest.getParameterFromContext('ContextVehicle', Constant.PRICECONSTANT.WELCOME_CONTEXT)
     ...
     * @memberof WebhookResponse
     */
    addGooglePayloadSuggestions(suggestions: any[], sendtodisplaydevice: boolean) {
        ...
        suggestions.forEach(suggestion => {
            let googleSuggestion = new GoogleSuggestion()
     ...
     * @memberof WebhookResponse
     */
    addGoogleCarousel(carousel: GoogleCarouselItem) {
        //if request is from google SCREEN device
        let source=this.webhookRequest.getParameterFromContext('source', Constant.Infobip.WELCOME_CONTEXT)
        if ((this.webhookRequest.getOriginalDetectIntentRequest() == "google" && this.webhookRequest.isScreenDevice()) || source=="WHATSAPP") {
            ...
            if (googleCarouselItemsLength == 0) {
                let carouselItem = new GoogleItem()
                carouselItem.carouselBrowse = new GoogleCarousel()
     ...
     * @memberof WebhookResponse
     */
    addGoogleTableCard(googlecarditem: GoogleTableCards) {
        //if request is from google SCREEN device
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google" && this.webhookRequest.isScreenDevice()) {
            ...
            if (googleItemLength == 0) {
                let tableItem = new GoogleItem()
                tableItem.tableCard = new GoogleTableCards()
    ...
    addGoogleTableCardCell(text: string) {
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google" && this.webhookRequest.isScreenDevice()) {
            let cellitem: GoogleTableCardsItemCell = new GoogleTableCardsItemCell()
            cellitem.text = text;
            return cellitem;
        }
    }

    addGoogleTableCards(rows: GoogleTableCardsItem[], columnProperties: GoogleTableCardsHeader[]) {
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google" && this.webhookRequest.isScreenDevice()) {
            let cellitem: GoogleTableCards = new GoogleTableCards()
            cellitem.rows = rows;
     ...
     * @memberof WebhookResponse
     */
    addGoogleList(googleListItem: GoogleListItem, listTitle) {
        //if request is from google SCREEN device
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google" && this.webhookRequest.isScreenDevice()) {
            //Initialize system intent
            if (!this.googlePayload.systemIntent) {
                this.googlePayload.systemIntent = new GoogleListIntent()
            }

            //Map system intent data to GoogleListData
            if (!this.googlePayload.systemIntent.data) {
                this.googlePayload.systemIntent.data = new GoogleListData(listTitle)
            }

            //Initialise list items
            if (this.googlePayload.systemIntent instanceof GoogleListIntent) {
    ...
    addGoogleSignInRequest() {
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google") {
            if (!this.googlePayload.systemIntent) {
                this.googlePayload.systemIntent = new GoogleSignInIntent()
            }
        }
    }
     ...
     * @memberof WebhookResponse
     */
    addGooglePermissionRequest(msg = "To know your location", permissions?: string[]) {
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google") {
            if (!this.googlePayload.systemIntent) {
                this.googlePayload.systemIntent = new GooglePermissionIntent()
                this.googlePayload.systemIntent.data = new GooglePermissionData(msg, permissions)
            }
        }
    }
    ...
    addGoogleLinkOutSuggestion(destinationName, URL) {
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google" || this.webhookRequest.isWhatsappRequest()) {
            if (!this.googlePayload.richResponse.linkOutSuggestion) {
                this.googlePayload.richResponse.linkOutSuggestion = new GoogleLinkOutSuggestion(destinationName, URL)
            } else {
    ...
    addGoogleBasicCard(title, subtitle,buttontitle,buttonurl,imageurl,imgalt) {
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google") {
            if (!this.googlePayload.richResponse.items) {
                let googleItem = new GoogleItem()
                let cellitem: GoogleBasicCard = new GoogleBasicCard()
     ...
     * @memberof WebhookResponse
     */
    addGoogleTransactionDecisionRequest(transactionName, merchantOrderId, buyerInfo: GoogleTransactionBuyerInfo, merchantInfo: GoogleTransactionMerchant, lineItmes: GoogleTransactionalLineItem[]) {
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google") {
            if (!this.googlePayload.systemIntent) {
                this.googlePayload.systemIntent = new GoogleTransactionDecisionIntent()
                let data = new GoogleTransactionDecisionData(transactionName)
                ...
                //Content details
                data.order.contents = new GoogleTransactionalContents()
    ...
    addGoogleOrderUpdate(reason, order) {
        if (this.webhookRequest.getOriginalDetectIntentRequest() == "google" && this.webhookRequest.isScreenDevice()) {
            ...
            let googleItem = new GoogleItem()
            googleItem.structuredResponse = new GoogleStructuredResponse()
            googleItem.structuredResponse.orderUpdateV3 = new GoogleOrderUpdate(reason)
     ...
     * @memberof WebhookResponse
     */
    private removeTypeFromResponse(message) {
        if (!message) {
            return null
        }
    ...
    getFulfillmentText() {
        ...
        else if (firstMessage instanceof DFCard) {
            return firstMessage.title || ""
        } else if (firstMessage instanceof CustomQuickReplies) {
            return "";
        }
     ...
     * @memberof WebhookResponse
     */
    getFulfillmentTextAll(number: number) {
     ...
     * @memberof WebhookResponse
     */
    addSeesionEntityType(entityName, entityValues: ISessisonEntity[], entityOverrideMode = 'ENTITY_OVERRIDE_MODE_OVERRIDE') {
        let sessionEntityType: ISessisonEntityType = {
     ...
     * @memberof WebhookResponse
     */
    createResponse(outputContext: IOutputContext[] = [], followupEvent = {}) {
        return {
...
export class PLATFORM {
    ...
    static UNKNOWN = "Unknown"  //This is not to add 'FACEBOOK' platfrom in responses where payload is being sent, without modifying GA responses- Specifically used for WYH bot
}

export interface ISessisonEntityType {
    name: string,
    entities: ISessisonEntity[],
    entityOverrideMode: string
}

export interface ISessisonEntity {
    value: string,
    synonyms: string[]
}
